...
|---|---|---|
| **[wiki-tools](plugins/wiki-tools)** | 1.8.1 | Trvalá kumulující se znalostní báze nad Obsidianem — ingest zdrojů, dotazy s citacemi, autonomní výzkum, psaní článků, health check vaultu |
| **[ai-review](plugins/ai-review)** | 1.4.0 | Druhý názor a cross-check napříč Claude, Codex (OpenAI) a Antigravity / agy (Google) — multi-model validace |

> Aktuální verze drží `plugins/<name>/.claude-plugin/plugin.json` — při release aktualizuj i tuto tabulku a badge v README pluginů.
...
| `/write-article [zadání]` | Investigativní článek: rešerše, striktní analýza, korektura češtiny a kontrola zdrojů | `write-article` |
| `/humanizer [text]` | Detekce a odstranění AI vzorců v textu (auto-detekce CZ / EN) | `humanizer` |
| `clean this url` | Strip ads, navigace a boilerplate před ingestem (defuddle) | `html-clean` |
| `/wiki-create` | Idempotentní inicializace wiki vaultu v aktuálním projektu | — (command) |
...
- **Antigravity CLI — `agy`** (Google; pro cross-check vždy s `Gemini …` modelem)
...
│           ├── second-opinion/
│           ├── double-cross-check/
│           └── references/      # sdílené CLI vzory a důvěryhodné zdroje
...
| Multi-model podpora | Claude Code, Codex CLI, Gemini CLI a další Agent Skills-kompatibilní nástroje | Claude Code, Codex CLI, Antigravity CLI (agy) |
